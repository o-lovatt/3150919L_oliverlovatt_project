"""
4-way ablation for Atlantic Puffin (strictly coastal cliff-breeder)
different from the general-purpose Barn Swallow test case
This shows whether DIST_TO_COAST_KM specifically earns it's place for a coastal species

Filename: verify_distance_features_puffin.py
...
def main(in_path):
    ...
    out_df.to_csv("bootstrap_distance_features_puffin_ablation.csv", index=False)
    print("\nSaved to bootstrap_distance_features_puffin_ablation.csv")


if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python verify_distance_features_puffin.py path/to/ebd_GB-SCT_filtered.txt")
        sys.exit(1)
    main(sys.argv[1])
